...
const PageHero = ({
  ...
  return (
    <section className="calm-grid-card overflow-hidden rounded-lg border border-ink/8 p-3 sm:p-4 lg:p-5">
      <div className={`relative z-10 grid min-w-0 gap-4 ${media ? "lg:grid-cols-[minmax(0,0.9fr)_minmax(300px,0.66fr)] lg:items-stretch" : ""}`}>
        <div className={`flex min-w-0 flex-col gap-4 ${media ? "lg:justify-center" : "md:flex-row md:items-end md:justify-between"}`}>
          <div className="min-w-0">
            ...
            <h1 className="section-title break-words text-[1.85rem] sm:text-[2.25rem] lg:text-[2.7rem]">{title}</h1>
            <p className="mt-2 max-w-3xl text-[0.98rem] font-semibold leading-7 text-ink/64 sm:text-[1.03rem]">{description}</p>
