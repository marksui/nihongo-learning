...
const ThemeSwitcher = () => {
  ...
  return (
    <section className="rounded-lg border border-ink/8 bg-paper/94 p-4 shadow-card" aria-labelledby="theme-title">
        ...
        {themes.map((theme) => {
          ...
          return (
            <button
              ...
              aria-pressed={active}
              className={`flex min-h-11 cursor-pointer items-center justify-between gap-3 rounded-md border px-3 py-2 text-left transition active:scale-[0.99] ${
                active
                  ? "border-matcha bg-matcha text-white shadow-card"
                  : "border-ink/10 bg-rice/35 text-ink hover:border-matcha/25 hover:bg-rice/65"
              }`}
            >
              <span className="min-w-0 truncate text-sm font-extrabold">{theme.name}</span>
