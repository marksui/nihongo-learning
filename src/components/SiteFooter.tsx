...
const SiteFooter = ({ currentPage, onNavigate }: SiteFooterProps) => {
  return (
    <footer className="mt-8 border-t border-ink/8 bg-paper/46 px-3 pb-28 pt-5 text-ink backdrop-blur sm:px-5 lg:px-8 lg:pb-8">
      <div className="mx-auto grid max-w-[84rem] gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.36fr)] lg:items-start">
        <section className="min-w-0 rounded-lg border border-ink/8 bg-paper/72 p-3" aria-labelledby="footer-title">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex min-w-0 items-center gap-2.5">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-xs font-bold text-ink/50">打开、点读、跟说</p>
              </div>
            </div>
          ...
          <div className="filter-scroll-row -mx-1 mt-3 flex gap-1.5 overflow-x-auto px-1 pb-1 sm:mx-0 sm:flex-wrap sm:overflow-visible sm:px-0" aria-label="页脚快速入口" role="group">
                ...
                <button
                  ...
                  className={`tap-surface shrink-0 cursor-pointer rounded-lg border px-3 py-2 text-sm font-bold transition active:scale-[0.99] ${
                    active
                      ...
                      : "border-ink/10 bg-paper/60 text-ink/64 hover:border-matcha/25 hover:bg-rice hover:text-ink"
                  }`}
                >
                  {item.label}
                </button>
